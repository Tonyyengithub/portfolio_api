...
def wipe_database
  Portfolio::Document.map(&:destroy)
  Portfolio::Project.map(&:destroy)
  Portfolio::Account.map(&:destroy)
end

def authenticate(account_data)
  Portfolio::AuthenticateAccount.call(
...
def authorization(account_data)
  ...
  { account: Portfolio::Account.first(username: account['username']),
